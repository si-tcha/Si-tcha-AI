...
export async function getB2BOffers(_req: AuthRequest, res: Response) {
  try {
...
export async function createB2BOffer(req: AuthRequest, res: Response) {
  if (req.user?.role !== 'seller' || !req.user.gicId) {
    return res.status(403).json({ message: 'Accès GIC requis.' });
  }

  const { title, type, category, priceOrExchange, gicName, location, contact } = req.body as {
    title?: string;
    type?: string;
    category?: string;
