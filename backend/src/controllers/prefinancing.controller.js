...
export async function getPrefinancingDeals(req, res) {
    if (!req.user) {
...
export async function createPrefinancingDeal(req, res) {
    if (!req.user) {
        return res.status(401).json({ message: 'Authentification requise.' });
    }
    const { gicName, buyerName, amountFcfa, inputDescription, reservedProduct, reservedVolumeKg } = req.body;
    if (!amountFcfa || !inputDescription?.trim()) {
        return res.status(400).json({ message: 'Montant et description des intrants sont requis.' });
    }
    try {
        let targetGicId = null;
        if (gicName?.trim()) {
