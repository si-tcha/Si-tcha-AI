...
export async function createBuyerOrders(req, res) {
    if (req.user?.role !== 'buyer' || !req.user.buyerId)
        return res.status(403).json({ message: 'Accès acheteur requis.' });
...
export async function getBuyerAlertPreferences(req, res) {
    if (req.user?.role !== 'buyer' || !req.user.buyerId)
        return res.status(403).json({ message: 'Accès acheteur requis.' });
    try {
        const buyerId = BigInt(req.user.buyerId);
...
export async function updateBuyerAlertPreferences(req, res) {
    if (req.user?.role !== 'buyer' || !req.user.buyerId)
        return res.status(403).json({ message: 'Accès acheteur requis.' });
