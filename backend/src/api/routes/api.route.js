...
function getEmojiForCategory(category, name) {
    ...
    if (category === 'Légumes')
        return '🥗';
    if (category === 'Céréales')
        return '🌾';
    if (category === 'Tubercules')
        return '🥔';
    if (category === 'Fruits')
        return '🍍';
    return '🌱';
...
router.get('/gic/profile', requireAuth, async (req, res) => {
    ...
    try {
        ...
        if (!gic)
            return res.status(404).json({ message: 'Profil GIC introuvable.' });
        const profile = {
            id: gic.id.toString(),
            name: gic.nom,
            identifiantREF: gic.identifiantREF,
...
router.put('/gic/profile', requireAuth, async (req, res) => {
...
router.get('/gic/harvests', requireAuth, async (req, res) => {
    if (req.user?.role !== 'seller' || !req.user.gicId)
        return res.status(403).json({ message: 'Accès GIC requis.' });
    try {
        const gicId = BigInt(req.user.gicId);
...
router.post('/gic/harvests', requireAuth, async (req, res) => {
    if (req.user?.role !== 'seller' || !req.user.gicId)
        return res.status(403).json({ message: 'Accès GIC requis.' });
    const { product, volume } = req.body;
    if (!product?.trim() || !Number.isFinite(Number(volume)))
        return res.status(400).json({ message: 'product et volume sont requis.' });
    try {
        const gicId = BigInt(req.user.gicId);
...
router.get('/gic/expenses', requireAuth, async (req, res) => {
    if (req.user?.role !== 'seller' || !req.user.gicId)
        return res.status(403).json({ message: 'Accès GIC requis.' });
    try {
        const gicId = BigInt(req.user.gicId);
...
router.post('/gic/expenses', requireAuth, async (req, res) => {
    if (req.user?.role !== 'seller' || !req.user.gicId)
        return res.status(403).json({ message: 'Accès GIC requis.' });
    const { label, amount, category } = req.body;
    if (!label?.trim() || !category?.trim() || !Number.isFinite(Number(amount)))
        return res.status(400).json({ message: 'label, category et amount sont requis.' });
    try {
        const gicId = BigInt(req.user.gicId);
...
router.post('/buyer/orders', requireAuth, async (req, res) => {
    if (req.user?.role !== 'buyer' || !req.user.buyerId)
        return res.status(403).json({ message: 'Accès acheteur requis.' });
    const { type, items } = req.body;
    if (!type || !items?.length)
        return res.status(400).json({ message: 'type et items sont requis.' });
    try {
        const buyerId = BigInt(req.user.buyerId);
        const created = [];
...
router.get('/buyer/alert-preferences', requireAuth, async (req, res) => {
    if (req.user?.role !== 'buyer' || !req.user.buyerId)
        return res.status(403).json({ message: 'Accès acheteur requis.' });
    try {
        const buyerId = BigInt(req.user.buyerId);
...
router.put('/buyer/alert-preferences', requireAuth, async (req, res) => {
    if (req.user?.role !== 'buyer' || !req.user.buyerId)
        return res.status(403).json({ message: 'Accès acheteur requis.' });
