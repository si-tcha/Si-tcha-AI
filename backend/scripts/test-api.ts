...
// --- FONCTIONS D'AIDE ---
const logStep = (step: number, description: string) => console.log(`\n--- ÉTAPE ${step}: ${description} ---`);
const logSuccess = (message: string, data?: any) => console.log(`✅ SUCCÈS: ${message}`, data ? JSON.stringify(data, null, 2) : '');
const logError = (message: string, error?: any) => console.error(`❌ ERREUR: ${message}`, error?.response?.data || error?.message || '');
const logInfo = (message: string) => console.log(`\nℹ️  ${message}`);
