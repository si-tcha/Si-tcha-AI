...
describe('propagation réelle des écritures privées', () => {
  ...
  it.each([
  ...
  ] as const)('propage SQLiteDiskIOWriteError depuis runSync via %s', async (method, key, value) => {
    ...
    if (method === 'saveParcels') {
      await expect(dbService.saveParcels(value, key)).rejects.toBe(diskError);
    } else {
  ...
  it.each([
    ['saveParcels', 'sitcha_parcels_seller_301_1', []],
    ['saveAgronomistHistory', 'sitcha_agro_history_seller_301_1', []],
  ] as const)('propage l’échec localStorage.setItem via %s', async (method, key, value) => {
    ...
    if (method === 'saveParcels') {
      await expect(dbService.saveParcels(value, key)).rejects.toBe(quotaError);
    } else {
